...
for color in g:colorexp#palette#colors
	let cc = str2nr(strpart(color.hex, 1), 16)
	let r = and(cc / 0x10000, 0xff)
	let g = and(cc / 0x100, 0xff)
	let b = and(cc, 0xff)
	let g:colorexp#palette#rgb += [[r, g, b]]
	let s:palette += [[r, g, b]]
	let g:colorexp#palette#name[tolower(color.name)] = color.color
...
"----------------------------------------------------------------------
function! colorexp#palette#bestfit8(r, g, b)
...
" find match in 8 colors
"----------------------------------------------------------------------
function! colorexp#palette#bestfit16(r, g, b)
...
" find match in 8 colors
"----------------------------------------------------------------------
function! colorexp#palette#bestfit256(r, g, b)
...
" consider config
"----------------------------------------------------------------------
function! colorexp#palette#bestfit(r, g, b)
	return s:bestfit_color(a:r, a:g, a:b, g:colorexp#palette#number)
endfunc
...
" matched
"----------------------------------------------------------------------
function! colorexp#palette#match(r, g, b)
	let r = (a:r < 256)? (a:r) : 255
	let g = (a:g < 256)? (a:g) : 255
	let b = (a:b < 256)? (a:b) : 255
	let key = (r * 4096 / 4) + (g * 64 / 4) + (b / 4)
	if !has_key(s:matched, key)
		let n = g:colorexp#palette#number
...
"----------------------------------------------------------------------
function! colorexp#palette#hex2rgb(hex)
	let [r, g, b] = [0, 0, 0]
...
"----------------------------------------------------------------------
function! colorexp#palette#hex2index(hex)
	let [r, g, b] = colorexp#palette#hex2rgb(a:hex)
	return colorexp#palette#match(r, g, b)
endfunc
...
" search name
"----------------------------------------------------------------------
function! colorexp#palette#name2index(name, ...)
	let head = strpart(a:name, 0, 1)
	if head == '#' || head == '('
		return colorexp#palette#hex2index(a:name)
	else
		...
		if exists('v:colornames')
			if has_key(v:colornames, name)
				...
				return colorexp#palette#hex2index(hex)
			endif
		endif
...
" alpha blend
"----------------------------------------------------------------------
function! colorexp#palette#blend(c1, c2, alpha)
	let c1 = a:c1
	let c2 = a:c2
	let alpha = a:alpha
	...
	let dst = colorexp#palette#hex2rgb(c1)
	let src = colorexp#palette#hex2rgb(c2)
...
function! colorexp#palette#color_match(r, g, b)
...
function! colorexp#palette#rgb_match(rgb) abort
	if a:rgb =~ '^#'
	...
	return colorexp#palette#color_match(r, g, b)
endfunc
...
" benchmark
"----------------------------------------------------------------------
function! colorexp#palette#timing()
	let ts = reltime()
	for i in range(256)
		call colorexp#palette#match(i, i, i)
	endfor
	let tt = reltime(ts)
...
if has('vim9script')
	...
	endfunc
	function! colorexp#palette#match(r, g, b)
		return s:palette9.Match(a:r, a:g, a:b, g:colorexp#palette#number)
	endfunc
	function! colorexp#palette#hex2rgb(hex)
